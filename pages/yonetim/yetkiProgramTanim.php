...
$tbl        = new YetkiProgram();
$gelen      = Crud::all($tbl);
$cmb        = Base::basitList(Crud::all(new Program()));
?>
            		...
            		<table id="tablebot" class="table table-bordered table-striped" >
            			<thead>
            				<tr>
            					...
            					<th class="bg-gray-700 text-gray-200 text-center align-middle">Program  </th>
            				</tr>
            			</thead>
            			<tbody>
                			<?php 
                			if ($gelen!= null){
                            	...
                            	<tr class="listeEleman" onclick="detayAc('<?= $gln->id ?>')">
                            		...
                            		<td class="text-center align-middle"><?= $gln->yetki_adi ?> </td>
                            		<td class="text-center align-middle"><?= $gln->program_id->ref->deger->program_adi ?> </td>
                            	</tr>
                                <?php
                    			}
                			}
        ...
        <div class="card shadow" >
            ...
            <div class="card-body">
                <form class="user">
                    ...
                    <div class="row mb-2">
                		<div class="col-lg-4 font-weight-bold text-gray-800 align-self-center  text-right">Yetki Adı :</div>
                		<div class="col-lg-8"><input type="text"  id="dznl_yetki_ad" class="form-control form-control-user"></div>
                	</div>
                    <div class="row mb-2">
                		<div class="col-lg-4 font-weight-bold text-gray-800 align-self-center  text-right">Program :</div>
                		<div class="col-lg-8">
<!--                 		<input type="text" id="dznl_program_id" class="form-control form-control-user"  > -->
                    		<select class=" custom-select form-control " id="dznl_program_id">
                    			<option class="dznl_val" value="" selected="selected">Seçiniz</option>
                    			<?php 
                    			 foreach ($cmb as $v){
                    			     echo '<option class="dznl_val" value="'.$v['id'].'">'.$v['program_adi'].($v['klasor'] == 'E' ? ' (K)' : '').'</option>';
                			     }
                                ?>
                            </select>
                		</div>
...
<script >
	...
	function detayAc(id){
	    ...
	    $.get( "<?php echo PREPATH.'post/genelPost.php?tur=getById&tablo='.get_class($tbl).'&id='?>"+id, function(data, status){
			if(status == "success"){
			    ...
			    $('#dznl_id').				val(obj.id);
			    $('#dznl_yetki_ad').		val(obj.yetki_adi);
			    $('#dznl_program_id').		val(obj.program_id.id);
    ...
    $("#dznl_btn").click(function(){
		...
		if (fromValid() ){
	        ...
	        $.post(link,
		        {
		    		id 			: $('#dznl_id').val(),
		    		yetki_adi 	: $('#dznl_yetki_ad').val(),
		    		program_id	: $('#dznl_program_id').val()
			    },
    ...
    function fromValid( sfr = false){
	    var snc1 = !sfr && ($('#dznl_yetki_ad').val() == null || $('#dznl_yetki_ad').val() == "");
	    var snc2 = !sfr && ($('#dznl_program_id').val() == null || $('#dznl_program_id').val() == "");
	    
		fromAlanValid('#dznl_yetki_ad',snc1) ;
		fromAlanValid('#dznl_program_id',snc2) ;
	...
	function formTemizle(valid = true){
	    $('#dznl_id').				val(null);
	    $('#dznl_yetki_ad').		val(null);
	    $('#dznl_program_id').		val(null);
