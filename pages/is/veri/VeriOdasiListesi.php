...
$mkLink    = Crud::getById(new Program() , 231 ) -> basit();
        	    ...
        	    if (obj.hata == true) {
        			...
        			if (ob != null && ob != ""){
            			ob.forEach(function(item){
            				...
            				'<a href="<?=PREPATH.$mkLink['program_link'].'?id=' ?>'+item.mkno+'" class="btn btn-warning ml-2" ><i class="fa fa-hand-pointer-o"></i><span class="text"> SEÇ</span></a>'+
                      		'</td>'+
                			'</tr>');
            			});
        				$('.tooltp').tooltip();
