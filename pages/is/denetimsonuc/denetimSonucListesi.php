...
$mkLink    = Crud::getById(new Program() , 243 ) -> basit();
?>
    ...
    <div class="col-lg-12 col-xl-12 pb-3">
        <div class="card shadow">
            <div class="card-header bg-gradient-primary py-3">
            	<h6 class="m-0 font-weight-bold text-gray-300">Denetim Sonuç Listesi</h6>
            </div>
            <div class="card-body">
...
<script >
    ...
    function listele(){
		var table = $('#tableLst');
		table.empty();
    	$.post("<?= PREPATH.'post/denetimSonucPost.php?tur=liste' ?>",
        {
        	ara_id		: $('#ara_id').val(),     
        	    ...
        	    if (obj.hata == true) {
        			...
        			if (ob != null && ob != ""){
            			ob.forEach(function(item){
            				...
            				'<a href="<?=PREPATH.$mkLink['program_link'].'?id=' ?>'+item.id+'" class="btn btn-warning ml-2" ><i class="fa fa-hand-pointer-o"></i><span class="text"> SEÇ</span></a>'+
                      		'</td>'+
                			'</tr>');
            			});
        				$('.tooltp').tooltip();
