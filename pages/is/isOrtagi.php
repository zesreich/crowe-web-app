...
$mstr   = Crud::getById(new IsOrtagi(), $id)->basit();
$tbl    = new Kullanici();

$gelen  = Crud::getSqlCok($tbl, Kullanici::KULLANICI_ISORTAG, array('isortagi'=>$id));

$mkLink = Crud::getById(new Program() , 204 ) -> basit();
$payDgr = Crud::getById(new Program() , 224 ) -> basit();
$prg    = Crud::getById(new Program() , 160 ) -> basit();
$pyls   = Crud::getById(new Program() , 221 ) -> basit();
$diger  = Crud::getById(new Program() , 226 ) -> basit();
      			...
      			<div class="col-xl-6">
                    <div class="card-body collapse" id="demo">
                        ...
                        <div class="row mb-2">
                    		...
                    		<div class="col-4" >
                    			<button type="button" class="btn btn-danger col-lg-12" data-toggle="modal" onclick="dataSil()" data-target="#myModal" id="sil_buton" >
                    				<i class="fas fa-exclamation"></i>      İş Ortağı Sil    <i class="fas fa-exclamation"></i>
                    			</button>
                      		</div>
                    	</div>
                    	<?php }?>
                        ...
                        <div class="row mb-2">
                    		...
                    		<div class="col-lg-8"><?=$mstr['vergi_daire_id'] != '' ? $mstr['vergi_daire_id']['adi'] : ''?></div>
                    	</div>
                        <div class="row mb-2">
                        ...
                        <div class="row mb-2">
                    		...
                    		<div class="col-lg-8"><?=$mstr['sektor_id'] != null ? $mstr['sektor_id']['adi'] : ''?></div>
                    	</div>
                        <div class="row mb-2">
                        ...
                        <div class="row mb-2">
                    		<div class="col-lg-6 font-weight-bold text-gray-800 align-self-center  text-center">
                    			<a href="<?=PREPATH.$pyls['program_link'].'?id='. $id ?>"  class="btn btn-primary col-lg-12" >İş Ortağı Paylaş</a>
                    		</div>
                        	<div class="col-lg-6">
                        		<a href="<?=PREPATH.$diger['program_link'].'?id='. $id ?>"  class="btn btn-primary col-lg-12" >Diğer İşler</a>
                        	</div>
                    	</div>
                    </div>
      			</div>
...
function dataSil(){
	$.post('<?php echo PREPATH.'pages/genel/kayitSil.php?tablo='.get_class(new IsOrtagi())?>',
        {
			tablo		: '<?= get_class(new IsOrtagi())?>',
			id			: <?=$id ?>,
			mesaj		: 'İş ortağını silmek istediğinize emin misiniz ?',
			donusLink	: '<?= PREPATH.'pages/is/isOrtagiListesi.php'?>',
			nkt			: '<?= PREPATH?>',
    	},
    ...
    function listele(){
		...
		var table = $('#tableLst');
		table.empty();
    	$.post("<?= PREPATH.'post/denetimPost.php?fnk=isOrtagiIsListesi&id='.$id ?>",
    	{
    		ara_id		: $('#ara_id').val(),     
        	    ...
        	    if (obj.hata == true) {
        			...
        			if (ob != null && ob != ""){
            			ob.forEach(function(item){
							...
							if (item.mk_id != null) {
                			  //  
                			    tklf = tklf + '<a href="<?=PREPATH.$mkLink['program_link'].'?id=' ?>'+item.teklif_id+'" class="btn btn-primary ml-2" ><i class="fa fa-hand-pointer-o"></i><span class="text">MK</span></a>';
							}
            			    ...
            			    $.post("<?= PREPATH.'post/denetimPost.php?fnk=paylasimButon'?>",
        				        {
        							id			: item.teklif_id,
        				    	},
        				    	function(data, status){
        				    		if(status == "success"){
										...
										$('#'+item.teklif_id+'_link').append('<a href="<?=PREPATH.$payDgr['program_link'].'?id=' ?>'+item.teklif_id+'" class="btn btn-'+color+' ml-2" ><i class="fa fa-hand-pointer-o"></i><span class="text">$</span></a>');
        				    		}else if(status == "error"){
        				    		    hataAc("Bilgi çekilemedi.");
        				    	    }
        				    	}
