...
$tbl = new Kullanici();
$cmb    = Base::basitList(Crud::getSqlCok(new GrupPrm(), GrupPrm::KULLANICI_GRUPLARI, array('kullaniciTurId'=> KullaniciTurPrm::MUSTERI)));
            ...
            <div class="card-body">
                <form class="user">
                	<div class="row mb-2">
                		...
                		<div class="col-6" id = "sil_Div" style="visibility: hidden;" >
                			...
                			<button type="button" class="btn btn-danger col-lg-12" data-toggle="modal" onclick="dataSil()" data-target="#myModal" id="sil_buton" >
                				<i class="fas fa-exclamation"></i>      Müşteri Kullanıcısını Sil    <i class="fas fa-exclamation"></i>
                			</button>
            				<?php }?>
                  		</div>
            		</div>
	            	...
	            	<div class="row mb-2">
                		...
                		<div class="col-lg-8"><input type="text" class="form-control form-control-user" value="Müşteri" disabled></div>
                		<input type="hidden" id="dznl_kullanici_tur" value="<?=KullaniciTurPrm::MUSTERI ?>" >
...
<script >
		...
		function dataSil(){
	    	$.post('<?php echo PREPATH.'pages/genel/kayitSil.php?tablo='.get_class($tbl).'&tur='.KullaniciTurPrm::MUSTERI?>',
                {
	    			tablo		: '<?= get_class($tbl)?>',
	    			id			: $('#dznl_id').val(),
	    			mesaj		: 'Müşteri kullanıcısını silmek istediğinize emin misiniz ?',
        			donusLink	: '<?= PREPATH.'pages/kullanici/musteriKullanici.php?id='.$Id?>',
					nkt			: '<?= PREPATH?>',
            	},
	   	...
	   	$("#dznl_btn").click(function(){
			...
			if (fromValid() ){
				...
				if ($('#dznl_id').val() == null || $('#dznl_id').val() == ""){
					link = "<?=PREPATH.'post/kullaniciPost.php?tur=musteriCreate&tablo='.get_class($tbl).'&mesaj=true' ?>";
				}else{
					link = "<?=PREPATH.'post/kullaniciPost.php?tur=musteriUpdate&tablo='.get_class($tbl).'&mesaj=true' ?>";
				}
		        $.post(link,
			        {
			    		id 					: $('#dznl_id').val(),
