...
$tbl = new Kullanici();
$gelen = Crud::all($tbl);
$klcCmb   = Base::basitList(Crud::all(new KullaniciTurPrm()));
            ...
            <div class="card-body">
                <form class="user">
	            	...
	            	<div class="row mb-2">
                		...
                		<div class="col-lg-8">
                    		<select class=" custom-select form-control " id="dznl_kullanici_tur" onchange="turSec()">
...
<script >
		...
		function detayAc(id){
			...
			$.get( "<?php echo PREPATH.'post/genelPost.php?tur=getById&tablo='.get_class($tbl).'&id='?>"+id, function(data, status){
				if(status == "success"){
				    ...
				    $('#dznl_kullanici_tur').	val(obj.grup_id.kullanici_tur_id.id);
				    turSec(obj.grup_id.id);
				    $('#dznl_id').				val(obj.id);
		...
		function turSec(id = null){
			var prg =$('#dznl_kullanici_tur').val();
			var table = $('#dznl_grup_id');
			table.find(".data").remove();
		    $.get( "<?php echo PREPATH.'post/yetkiPost.php?tur=kullaniciTurGruplari&kullaniciTurId='?>"+prg, function(data, status){
    			if(status == "success"){
    			    var objx = JSON.parse(data);
    			    if (objx.hata == false) {
         			    var obj = JSON.parse(objx.icerik);
            			if (obj != null && obj != ""){
                			obj.forEach(function(item){
                				table.append(
                					'<option class="dznl_val data" value="'+item.id+'">'+item.adi+'</option>'
                				);
                			});
            			}
        		    }
	   	...
	   	$("#dznl_btn").click(function(){
			...
			if (fromValid() ){
				...
				if ($('#dznl_id').val() == null || $('#dznl_id').val() == ""){
					link = "<?=PREPATH.'post/kullaniciPost.php?tur=create&tablo='.get_class($tbl).'&mesaj=true' ?>";
				}else{
					link = "<?=PREPATH.'post/kullaniciPost.php?tur=update&tablo='.get_class($tbl).'&mesaj=true' ?>";
				}
		        $.post(link,
			        {
			    		id 					: $('#dznl_id').val(),
	    ...
	    function fromValid( sfr = false){
		    ...
		    var snc4 = !sfr && ($('#dznl_kullanici_tur').val() == null || $('#dznl_kullanici_tur').val() == "");
			...
			fromAlanValid('#dznl_kullanici_tur',snc4) ;
			fromAlanValid('#dznl_grup_id',snc5) ;
			    
			if (snc1 || snc2 || snc3 || snc4 || snc5) {
