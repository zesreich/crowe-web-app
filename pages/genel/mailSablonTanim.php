            		...
            		<table id="tablebot" class="table table-bordered table-striped" >
            			<thead>
            				<tr>
            					...
            					<th class="bg-gray-700 text-gray-200 text-center align-middle">Key</th>
            				</tr>
            			</thead>
            			<tbody>
                			<?php 
                			if ($gelen!= null){
                            	...
                            	<tr class="listeEleman" onclick="detayAc('<?= $gln->id ?>')">
                            		...
                            		<td class="text-center align-middle"><?= $gln->skey ?> </td>
                            	</tr>
                                <?php
                    			}
                			}
        ...
        <div class="card shadow" >
            ...
            <div class="card-body">
                <form class="user">
                    ...
                    <div class="row mb-2">
                		<div class="col-lg-4 font-weight-bold text-gray-800 align-self-center  text-right">Key :</div>
                		<div class="col-lg-8"><input type="text"  id="dznl_skey" class="form-control form-control-user"></div>
                	</div>
                    <div class="row mb-2">
                		<div class="col-lg-4 font-weight-bold text-gray-800 align-self-center  text-right">Başlık :</div>
                		<div class="col-lg-8"><input type="text" id="dznl_baslik" class="form-control "></div>
                	</div>
                    <div class="row mb-2">
                		<div class="col-lg-4 font-weight-bold text-gray-800 align-self-center  text-right">Mesaj :</div>
                		<div class="col-lg-8"><textarea rows="8"  id="dznl_mesaj" class="form-control "></textarea></div>
                	</div>
                    <div class="row mb-2">
                		<div class="col-lg-4 font-weight-bold text-gray-800 align-self-center  text-right">Açıklama :</div>
                		<div class="col-lg-8"><textarea rows="5"  id="dznl_aciklama" class="form-control "></textarea></div>
                	</div>
                    <div class="row mb-2">
...
<script >
	...
	function detayAc(id){
	    ...
	    $.get( "<?php echo PREPATH.'post/genelPost.php?tur=getById&tablo='.get_class($tbl).'&id='?>"+id, function(data, status){
			if(status == "success"){
			    ...
			    $('#dznl_id').				val(obj.id);
			    $('#dznl_skey').			val(obj.skey          );
			    $('#dznl_baslik').			val(obj.baslik   	  );
			    $('#dznl_mesaj').			val(obj.mesaj   	  );
    ...
    $("#dznl_btn").click(function(){
		...
		if (fromValid() ){
			...
			console.log($('#dznl_id').val());
			console.log($('#dznl_skey').val());
			console.log($('#dznl_baslik').val());
			console.log($('#dznl_mesaj').val());
			console.log($('#dznl_aciklama').val());
	        $.post(link,
		        {
		    		id 			: $('#dznl_id').val(),
		    		skey       	: $('#dznl_skey').val(),
		    		baslik		: $('#dznl_baslik').val(),
		    		mesaj		: $('#dznl_mesaj').val(),
    ...
    function fromValid( sfr = false){
	    var snc1 = !sfr && ($('#dznl_skey').val() == null || $('#dznl_skey').val() == "");
	    var snc2 = !sfr && ($('#dznl_baslik').val() == null || $('#dznl_baslik').val() == "");
	    var snc3 = !sfr && ($('#dznl_mesaj').val() == null || $('#dznl_mesaj').val() == "");
	    var snc4 = !sfr && ($('#dznl_aciklama').val() == null || $('#dznl_aciklama').val() == "");
	    
		fromAlanValid('#dznl_skey',snc1) ;
		fromAlanValid('#dznl_baslik',snc2) ;
		fromAlanValid('#dznl_mesaj',snc3) ;
		fromAlanValid('#dznl_aciklama',snc4) ;
		    
		if (snc1 || snc2 || snc3 || snc4) {
