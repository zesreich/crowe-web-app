...
output_path = os.path.join('html', 'denetim-rehberi.html')

print("=" * 60)
print("DENETİM REHBERİ HTML OLUŞTURUCU")
print("=" * 60)
...
<head>
    ...
    <title>Denetim Rehberi - B10 OD Dosya İncelemesi</title>
...
<body>
    ...
    for sheet_idx, sheet_name in enumerate(wb.sheetnames, 1):
        print(f"\n📊 Sayfa {sheet_idx}/{total_sheets}: {sheet_name}")
        ws = wb[sheet_name]
        ...
        if len(df) > 0 and len(df.columns) > 0:
            for col_idx in range(min(20, len(df.columns))):  # Limit to 20 columns
                html_parts.append(f'<th>{get_column_letter(col_idx + 1)}</th>')
        else:
            html_parts.append('<th>A</th>')
        ...
        # Add rows (limit to 100 rows for performance)
        max_rows = min(100, len(df)) if len(df) > 0 else 0
        for row_idx in range(max_rows):
            html_parts.append('<tr>')
            max_cols = min(20, len(df.columns)) if len(df) > 0 else 1
            for col_idx in range(max_cols):
                ...
                cell_value = cell_value[:100]  # Limit cell value length
                
                is_validation = cell_ref in validations
                validation_class = 'validation-cell' if is_validation else ''
                
                html_parts.append(f'<td class="{validation_class}">')
    ...
    html_parts[1] = html_parts[1].replace('id="stats-row">', f'id="stats-row">{stats_html}')
    ...
    print("\n" + "=" * 60)
    print(f"✅ HTML sayfası oluşturuldu: {output_path}")
