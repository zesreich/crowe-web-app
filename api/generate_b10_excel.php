...
try {
    // Composer autoload - Vercel'de vendor klasörü api/ veya root'ta olabilir
    $autoloadPaths = [
    ...
    if (!$autoloadPath) {
        throw new Exception('Composer autoload bulunamadı. PhpSpreadsheet kurulu olmalı.');
    }
    ...
    // Şablon dosyasının yolu - Vercel'de public klasöründe veya api/ içinde
    $templatePaths = [
        __DIR__ . '/../template/B10_ODDI.xlsx',
        __DIR__ . '/../public/template/B10_ODDI.xlsx',
        __DIR__ . '/template/B10_ODDI.xlsx',
        '/var/task/template/B10_ODDI.xlsx', // Vercel Lambda path
    ];
    ...
    if (!$templatePath) {
        throw new Exception('Şablon dosyası bulunamadı. B10_ODDI.xlsx dosyasının template/ klasöründe olduğundan emin olun.');
    }
    ...
    // Geçici dosya oluştur (Vercel'de /tmp kullanılabilir)
    $tempDir = sys_get_temp_dir();
    if (!is_writable($tempDir)) {
        $tempDir = '/tmp';
    }
    $tempFile = $tempDir . '/' . uniqid('b10_', true) . '.xlsx';
